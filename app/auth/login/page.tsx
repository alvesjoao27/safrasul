  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-center bg-[#F5F2EB] px-4 py-12">
      ...
      <div className="w-full max-w-[360px] bg-white rounded-2xl border border-stone-200 shadow-sm px-8 py-8">
        <h2 className="text-base font-medium text-stone-800 mb-6">Entrar na sua conta</h2>
        ...
        <form onSubmit={handleSubmit(onSubmit)} noValidate className="flex flex-col gap-5">
          <div className="flex flex-col gap-1.5">
            ...
            <input
              ...
              className={`w-full rounded-lg border px-3.5 py-2.5 text-sm text-stone-900
                placeholder:text-stone-400 outline-none transition
                focus:ring-2 focus:ring-[#2D5016]/25 focus:border-[#2D5016]
                ${errors.email ? 'border-red-400 bg-red-50/50' : 'border-stone-300 hover:border-stone-400'}`}
              {...register('email')}
            />
          ...
          <div className="flex flex-col gap-1.5">
            <div className="flex items-center justify-between">
              ...
              <a href="/auth/recuperar" className="text-xs text-[#3a6620] hover:underline">Esqueci minha senha</a>
            </div>
            <input
              ...
              className={`w-full rounded-lg border px-3.5 py-2.5 text-sm text-stone-900
                placeholder:text-stone-400 outline-none transition
                focus:ring-2 focus:ring-[#2D5016]/25 focus:border-[#2D5016]
                ${errors.senha ? 'border-red-400 bg-red-50/50' : 'border-stone-300 hover:border-stone-400'}`}
              {...register('senha')}
            />
        ...
        <p className="text-center text-sm text-stone-500">
          Ainda não tem conta?{' '}
          <a href="/auth/cadastro" className="text-[#2D5016] font-medium hover:underline">
            Criar conta grátis
          </a>
        </p>
      </div>
