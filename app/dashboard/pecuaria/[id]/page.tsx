...
export default function AnimalPage() {
  ...
  async function salvarEdicao() {
    ...
    if (error) {
      setErroEd('Erro ao salvar. Tente novamente.')
      setSalvandoEd(false)
      return
    }
  ...
  return (
    <div className="min-h-screen bg-[#F5F2EB]">
      ...
      <main className="max-w-lg mx-auto px-4 py-5 space-y-4">
          ...
          {(['perfil', 'historico'] as const).map(aba => (
            <button key={aba} onClick={() => setAbaAtiva(aba)}
              className={`flex-1 py-3 text-sm font-medium transition
                ${abaAtiva === aba ? 'text-[#2D5016] border-b-2 border-[#2D5016] bg-[#2D5016]/5' : 'text-stone-400 hover:text-stone-600'}`}>
        ...
        {abaAtiva === 'historico' && (
          <div className="space-y-4">
            <button onClick={() => setNovoEvento(!novoEvento)}
            ...
            {novoEvento && (
              <div className="bg-white rounded-2xl border border-stone-200 p-5 space-y-4">
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <input type="text" placeholder="Ex: Vacina febre aftosa..." value={descEvento} onChange={e => setDescEvento(e.target.value)}
                ...
                <div className="flex gap-2">
                  <button onClick={() => setNovoEvento(false)}
                    className="flex-1 rounded-lg border border-stone-300 text-stone-600 text-sm py-2.5 hover:bg-stone-50 transition">
                    Cancelar
                  </button>
