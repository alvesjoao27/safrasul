...
export default function DashboardPage() {
  ...
  async function carregarDados() {
    ...
    const { data: saf } = await supabase
      .from('safras')
      .select('id, nome, status, culturas(nome)')
      .eq('fazenda_id', faz.id)
    ...
    try {
      ...
      if (geo.results?.[0]) {
        ...
        const desc = code <= 1 ? 'Céu limpo' : code <= 3 ? 'Parcialmente nublado' : code <= 48 ? 'Nublado / Névoa' : code <= 67 ? 'Chuva' : code <= 77 ? 'Neve' : 'Tempestade'
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#F5F2EB] flex items-center justify-center">
      <div className="flex flex-col items-center gap-3">
        <svg width="40" height="40" viewBox="0 0 48 48" fill="none" className="animate-pulse">
  ...
  return (
    <div className="min-h-screen bg-[#F5F2EB]">
      ...
      <main className="max-w-2xl mx-auto px-4 py-6 space-y-6">
        ...
        {/* Saudação */}
        <div>
          <h1 className="text-xl font-semibold text-[#1E3A0F]">
            Bom dia, {nomeUser}! 👋
          </h1>
        ...
        <section>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-white rounded-xl border border-stone-200 p-4">
              ...
              <p className="text-base font-semibold text-emerald-600">{fmt(kpis.receitas)}</p>
            </div>
            <div className="bg-white rounded-xl border border-stone-200 p-4">
              <p className="text-xs text-stone-400 mb-1">Despesas</p>
              <p className="text-base font-semibold text-red-500">{fmt(kpis.despesas)}</p>
            </div>
            <div className={`rounded-xl border p-4 ${kpis.saldo >= 0 ? 'bg-emerald-50 border-emerald-200' : 'bg-red-50 border-red-200'}`}>
              ...
              <p className={`text-base font-semibold ${kpis.saldo >= 0 ? 'text-emerald-700' : 'text-red-600'}`}>
                {fmt(kpis.saldo)}
              </p>
            </div>
          </div>
        ...
        <section>
          ...
          {safras.length === 0 ? (
            <div className="bg-white rounded-xl border border-dashed border-stone-300 p-6 text-center">
              ...
              <a href="/dashboard/lavoura" className="text-xs text-[#2D5016] font-medium mt-1 inline-block hover:underline">
                Cadastrar safra →
              </a>
            </div>
          ) : (
            <div className="grid grid-cols-2 gap-3">
              {safras.map(s => (
                <div key={s.id} className="bg-white rounded-xl border border-stone-200 p-4">
                  ...
                  <span className="inline-block mt-2 text-xs bg-lime-100 text-lime-700 px-2 py-0.5 rounded-full">
                    Em andamento
                  </span>
                </div>
              ))}
          ...
          <div className="grid grid-cols-3 gap-2">
            {CUIDADOS_REBANHO.map(c => (
                ...
                className="bg-white rounded-xl border border-stone-200 p-3 text-center
                  hover:border-[#2D5016]/30 hover:bg-[#2D5016]/5 transition"
              >
